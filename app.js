...
function buildImagePrompt(look, selection) {
  ...
  const presLabel =
    selection.presentation === "feminine" ? "feminine-presenting" :
    selection.presentation === "masculine" ? "masculine-presenting" :
    "androgynous";

  return [
    "editorial fashion photograph, full body, single person, centered",
    `${presLabel} model dressed for a ${occLabel}`,
    `wearing ${look.clothing.name}`,
    `${look.hairstyle.name} hairstyle`,
    `${look.jewellery.name}`,
    `${look.makeup.name} beauty look`,
    `${weatherLabel.toLowerCase()} weather setting`,
    `${styleLabel.toLowerCase()} aesthetic`,
    "soft natural lighting, clean neutral studio background, magazine cover style, high quality, sharp focus",
  ].join(", ");
}

function buildImageUrl(prompt, seed = 1) {
  // Use the default (fast turbo) model — flux is slow and returns a placeholder
  // HTML page during generation that the browser then blocks via ORB.
